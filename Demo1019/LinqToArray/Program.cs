...
namespace LinqToArray
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Returned results from lnghand verson");
        ...
        static string[] QueryOverStringLongHand(string[] s)
        {
            ...
            count = 0;
            foreach (string s1 in outputArray)
            {
                if (s1 != null)
                {
        ...
        static List<string> QueryOverStrings(string[] inputArray)
        {
            ...
            Console.WriteLine("  Immediate results using LINQ query after change to tdata");
            foreach (var s in subset)
            {
                Console.WriteLine("Item: {0}", s);
            }
